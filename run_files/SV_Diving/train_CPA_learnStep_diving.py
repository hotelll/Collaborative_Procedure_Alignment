...
def train():
    ...
    for epoch in range(start_epoch, cfg.TRAIN.MAX_EPOCH):
        ...
        for iter, sample in enumerate(train_loader):
            ...
            pn_coeff = pair_labels * 2 - 1
            loss_align = (frame2step_dist * pn_coeff).clamp(min=-10).mean(0)
            loss_step = (loss_step * pair_labels).mean(0)
...
def parse_args():
    ...
    args = parser.parse_args([
        '--config', 'configs/train_adaK_learnGaussStep_diving_config.yml'
    ])
    return args
