...
def elastic_step_mining(seq_features1, seq_features2, total_step, matched_step):
    max_jumps = total_step - matched_step
    seq_features1 = seq_features1[:, 1:]
    seq_features2 = seq_features2[:, 1:]
    
    (B, T, C), R, device = seq_features1.shape, 5+1, seq_features1.device
    pred = torch.cosine_similarity(seq_features1.unsqueeze(2), seq_features2.unsqueeze(1), dim=-1)
    pred = pred.cumsum(-2).cumsum(-1)
    ...
    for k in range(1, T):
        ...
        if k == 1:
            ...
            D03_candidate = (tmp_0 * k + block_mat) / (k+1)
            D[:, 0, 3] = D03_candidate.flatten(3).max(-1).values
            D_ind[:, 0, 3] = D03_candidate.flatten(3).max(-1).indices
            # ---------------------------------------------------------------------------------------------
            D02_candidate = tmp_0 + empty_mat
            D12_candidate = (tmp_1 * (k-1) + block_mat) / k
            D13_candidate = torch.cat([D02_candidate.flatten(3), D12_candidate.flatten(3)], dim=-1)
            D[:, 1, 3] = D13_candidate.flatten(3).max(-1).values
            D_ind[:, 1, 3] = D13_candidate.flatten(3).max(-1).indices
            # ---------------------------------------------------------------------------------------------
            D12_candidate = tmp_1 + empty_mat
            D22_candidate = (tmp_2 * (k-2) + block_mat) / (k-1)
            D23_candidate = torch.cat([D12_candidate.flatten(3), D22_candidate.flatten(3)], dim=-1)
            D[:, 2, 3] = D23_candidate.flatten(3).max(-1).values
            D_ind[:, 2, 3] = D23_candidate.flatten(3).max(-1).indices + 256
            # ---------------------------------------------------------------------------------------------
            D33_candidate = tmp_2 + empty_mat
            D[:, 3, 3] = D33_candidate.flatten(3).max(-1).values
            D_ind[:, 3, 3] = D33_candidate.flatten(3).max(-1).indices + 256 * 2
        
        elif k == 4:
            D04_candidate = (tmp_0 * k + block_mat) / (k+1)
            D[:, 0, 4] = D04_candidate.flatten(3).max(-1).values
            D_ind[:, 0, 4] = D04_candidate.flatten(3).max(-1).indices
            # ---------------------------------------------------------------------------------------------
            D03_candidate = tmp_0 + empty_mat
            D13_candidate = (tmp_1 * (k-1) + block_mat) / k
            D14_candidate = torch.cat([D03_candidate.flatten(3), D13_candidate.flatten(3)], dim=-1)
            D[:, 1, 4] = D14_candidate.flatten(3).max(-1).values
            D_ind[:, 1, 4] = D14_candidate.flatten(3).max(-1).indices
            # ---------------------------------------------------------------------------------------------
            D13_candidate = tmp_1 + empty_mat
            D23_candidate = (tmp_2 * (k-2) + block_mat) / (k-1)
            D24_candidate = torch.cat([D13_candidate.flatten(3), D23_candidate.flatten(3)], dim=-1)
            D[:, 2, 4] = D24_candidate.flatten(3).max(-1).values
            D_ind[:, 2, 4] = D24_candidate.flatten(3).max(-1).indices + 256
            # ---------------------------------------------------------------------------------------------
            D23_candidate = tmp_2 + empty_mat
            D33_candidate = (tmp_3 * (k-3) + block_mat) / (k-2)
            D34_candidate = torch.cat([D23_candidate.flatten(3), D33_candidate.flatten(3)], dim=-1)
            D[:, 3, 4] = D34_candidate.flatten(3).max(-1).values
            D_ind[:, 3, 4] = D34_candidate.flatten(3).max(-1).indices + 256 * 2
            # ---------------------------------------------------------------------------------------------
            D44_candidate = tmp_3 + empty_mat
            D[:, 4, 4] = D44_candidate.flatten(3).max(-1).values
            D_ind[:, 4, 4] = D44_candidate.flatten(3).max(-1).indices + 256 * 3
        
        elif k == 5:
            D05_candidate = (tmp_0 * k + block_mat) / (k+1)
            D[:, 0, 5] = D05_candidate.flatten(3).max(-1).values
            D_ind[:, 0, 5] = D05_candidate.flatten(3).max(-1).indices
            # ---------------------------------------------------------------------------------------------
            D04_candidate = tmp_0 + empty_mat
            D14_candidate = (tmp_1 * (k-1) + block_mat) / k
            D15_candidate = torch.cat([D04_candidate.flatten(3), D14_candidate.flatten(3)], dim=-1)
            D[:, 1, 5] = D15_candidate.flatten(3).max(-1).values
            D_ind[:, 1, 5] = D15_candidate.flatten(3).max(-1).indices
            # ---------------------------------------------------------------------------------------------
            D14_candidate = tmp_1 + empty_mat
            D24_candidate = (tmp_2 * (k-2) + block_mat) / (k-1)
            D25_candidate = torch.cat([D14_candidate.flatten(3), D24_candidate.flatten(3)], dim=-1)
            D[:, 2, 5] = D25_candidate.flatten(3).max(-1).values
            D_ind[:, 2, 5] = D25_candidate.flatten(3).max(-1).indices + 256
            # ---------------------------------------------------------------------------------------------
            D24_candidate = tmp_2 + empty_mat
            D34_candidate = (tmp_3 * (k-3) + block_mat) / (k-2)
            D35_candidate = torch.cat([D24_candidate.flatten(3), D34_candidate.flatten(3)], dim=-1)
            D[:, 3, 5] = D35_candidate.flatten(3).max(-1).values
            D_ind[:, 3, 5] = D35_candidate.flatten(3).max(-1).indices + 256 * 2
            # ---------------------------------------------------------------------------------------------
            D34_candidate = tmp_3 + empty_mat
            D44_candidate = (tmp_4 * (k-4) + block_mat) / (k-3)
            D45_candidate = torch.cat([D34_candidate.flatten(3), D44_candidate.flatten(3)], dim=-1)
            D[:, 4, 5] = D45_candidate.flatten(3).max(-1).values
            D_ind[:, 4, 5] = D45_candidate.flatten(3).max(-1).indices + 256 * 3
            # ---------------------------------------------------------------------------------------------
            D55_candidate = tmp_4 + empty_mat
            D[:, 5, 5] = D55_candidate.flatten(3).max(-1).values
            D_ind[:, 5, 5] = D55_candidate.flatten(3).max(-1).indices + 256 * 4
            # ---------------------------------------------------------------------------------------------
        
        else:
            D06_candidate = (tmp_0 * k + block_mat) / (k+1)
            D[:, 0, k] = D06_candidate.flatten(3).max(-1).values
            D_ind[:, 0, k] = D06_candidate.flatten(3).max(-1).indices
            # ---------------------------------------------------------------------------------------------
            D05_candidate = tmp_0 + empty_mat
            D15_candidate = (tmp_1 * (k-1) + block_mat) / k
            D16_candidate = torch.cat([D05_candidate.flatten(3), D15_candidate.flatten(3)], dim=-1)
            D[:, 1, k] = D16_candidate.flatten(3).max(-1).values
            D_ind[:, 1, k] = D16_candidate.flatten(3).max(-1).indices
            # ---------------------------------------------------------------------------------------------
            D15_candidate = tmp_1 + empty_mat
            D25_candidate = (tmp_2 * (k-2) + block_mat) / (k-1)
            D26_candidate = torch.cat([D15_candidate.flatten(3), D25_candidate.flatten(3)], dim=-1)
            D[:, 2, k] = D26_candidate.flatten(3).max(-1).values
            D_ind[:, 2, k] = D26_candidate.flatten(3).max(-1).indices + 256
            # ---------------------------------------------------------------------------------------------
            D25_candidate = tmp_2 + empty_mat
            D35_candidate = (tmp_3 * (k-3) + block_mat) / (k-2)
            D36_candidate = torch.cat([D25_candidate.flatten(3), D35_candidate.flatten(3)], dim=-1)
            D[:, 3, k] = D36_candidate.flatten(3).max(-1).values
            D_ind[:, 3, k] = D36_candidate.flatten(3).max(-1).indices + 256 * 2
            # ---------------------------------------------------------------------------------------------
            D35_candidate = tmp_3 + empty_mat
            D45_candidate = (tmp_4 * (k-4) + block_mat) / (k-3)
            D46_candidate = torch.cat([D35_candidate.flatten(3), D45_candidate.flatten(3)], dim=-1)
            D[:, 4, k] = D46_candidate.flatten(3).max(-1).values
            D_ind[:, 4, k] = D46_candidate.flatten(3).max(-1).indices + 256 * 3
            # ---------------------------------------------------------------------------------------------
            D45_candidate = tmp_4 + empty_mat
            D55_candidate = (tmp_5 * (k-5) + block_mat) / (k-4)
            D56_candidate = torch.cat([D45_candidate.flatten(3), D55_candidate.flatten(3)], dim=-1)
            D[:, 5, k] = D56_candidate.flatten(3).max(-1).values
            D_ind[:, 5, k] = D56_candidate.flatten(3).max(-1).indices + 256 * 4
            # ---------------------------------------------------------------------------------------------
